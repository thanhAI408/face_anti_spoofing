...
model = load_model("liveness.h5")  # Đảm bảo dùng file đúng định dạng
...
while True:
    # Doc anh tu webcam
    frame = vs.read()
    frame = cv2.flip(frame, 1)  # Lật ngang để hình không bị ngược
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args["confidence"]:
            ...
            # Kiểm tra xem vùng khuôn mặt có bị lỗi không
            if face.shape[0] == 0 or face.shape[1] == 0:
                continue  # Bỏ qua frame này và xử lý frame tiếp theo
